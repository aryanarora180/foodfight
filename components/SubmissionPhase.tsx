    ...
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <div className="felt-panel neon-border rounded-3xl p-6">
        <h2 className="font-display mb-1 text-xl text-gold">
          {mine ? "Update your pick" : "Toss in your pick"}
        </h2>
        <p className="mb-5 text-sm text-white/50">
          One restaurant per person. You can change it until voting starts.
        </p>
        <form onSubmit={submit}>
          ...
          />
          <p className="mb-5 text-xs text-white/40">Link to the menu so everyone can scope it out.</p>
          ...
          >
            {loading ? "LOCKING IN…" : mine ? "UPDATE MY PICK ✏️" : "LOCK IN MY PICK 🎲"}
          </motion.button>
        </form>
      </div>
      ...
      <div>
        <h3 className="font-display mb-3 text-lg text-cyan">
          On the table ({state.restaurants.length})
        </h3>
        {state.restaurants.length === 0 ? (
          <p className="text-white/40">Nobody has thrown in a pick yet. Be the first!</p>
        ) : (
            ...
            <AnimatePresence>
              {state.restaurants.map((r) => (
                ...
                >
                  ...
                  <p className="mt-2 text-xs text-cyan/70 underline">view menu →</p>
                </motion.a>
              ))}
            </AnimatePresence>
          </div>
